...
func TestConvert(t *testing.T) {
	...
	png, err := image.ConvertDDS(data, image.PNG, image.WithTempDir(tmpDir))
	assert.NoError(t, err, "converting to PNG")
	os.WriteFile("./sample/loadingimageancient02.png", png, os.ModePerm)

	webp, err := image.ConvertDDS(data, image.WEBP, image.WithTempDir(tmpDir))
	...
	png, err = image.ConvertDDS(data, image.PNG, image.WithTempDir(tmpDir), image.WithNormalMap(true))
	assert.NoError(t, err, "converting to PNG")
	os.WriteFile("./sample/fish_ddna.png", png, os.ModePerm)

	webp, err = image.ConvertDDS(data, image.WEBP, image.WithTempDir(tmpDir), image.WithNormalMap(true))
	assert.NoError(t, err, "converting to WEBP")
