...
export class AdbPlayer {
  #loadingAssets: Record<string, Promise<ModelAsset>> = {}
  ...
  public async reset(game: GameServiceContainer, target: Object3D) {
    this.game = game
    this.target = target
    ...
    await Promise.all(
      layer.sequence.map(async (animation) => {
        if (animation.file) {
          ...
          animation.animation = buildAnimationGroup(animation.name, asset, this.target)
        }
      }),
    )
...
function buildAnimationGroup(name: string, animationAsset: any, target: Object3D) {
  console.log('buildAnimationGroup', name, animationAsset, target)
