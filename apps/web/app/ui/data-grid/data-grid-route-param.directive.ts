...
import { NEVER, distinctUntilChanged, filter, map, merge, of, startWith, switchMap, takeUntil, tap } from 'rxjs'
import { DataGridStore } from './data-grid.store'

@Directive({
  standalone: true,
})
export class DataTableRouterDirective implements OnInit {
  public constructor(private route: ActivatedRoute, private router: Router, private store: DataGridStore<unknown>) {
    //
  }

  public ngOnInit(): void {
    this.store.selectionParam$
  ...
  private handleSelectionChange(param: string) {
    ...
    return this.store.selectedItemIds$.pipe(
      map((it) => it?.[0]),
