...
export interface TrackingCellParams<T> {
  getId: (item: T) => string
...
export class TrackingCell<T> implements ICellRendererComp<T> {
  public static params<T>(params: TrackingCellParams<T>) {
    return params
  }
  ...
  private params: TrackingCellParams<T>
  ...
  public init(params: ICellRendererParams<T, any> & TrackingCellParams<T>): void | AgPromise<void> {
    this.params = params
    ...
    this.item$
      ...
      .subscribe((it) => {
        ...
        this.trackedValue = cleanValue(it?.meta?.[this.params.mode])
        this.updateUI()
      })
  }
  ...
  private write() {
    if (this.trackedId) {
      this.params.pref.merge(this.trackedId, {
        [this.params.mode]: this.trackedValue,
      })
    }
  }
}
