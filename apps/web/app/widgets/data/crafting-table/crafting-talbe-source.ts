...
@Injectable()
export class CraftingTableSource extends DataTableSource<CraftingTableRecord> {
  ...
  public override entityID(item: CraftingTableRecord): string {
    return item.RecipeID
  }

  public override entityCategories(item: CraftingTableRecord): DataTableCategory[] {
    if (!item.Tradeskill) {
      return null
    }
    return [
  ...
  public override gridOptions(): GridOptions<CraftingTableRecord> {
    const build = this.config?.gridOptions || buildOptions
    return build(this.utils)
  }

  public override connect(): Observable<CraftingTableRecord[]> {
