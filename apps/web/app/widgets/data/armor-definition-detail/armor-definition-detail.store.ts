...
export interface ArmorDefinitionDetailState {
  recordId: string
}
export const ArmorDefinitionDetailStore = signalStore(
  withState<ArmorDefinitionDetailState>({
    recordId: null,
  }),
  withMethods((state) => {
    return {
  ...
  withProps(({ recordId }) => {
    ...
    const dataResource = resource({
      ...
      loader: async ({ params: { recordId } }) => {
        const record = await db.armorItemsById(recordId)
        return {
          record,
        }
      },
...
function selectProperties(item: ArmorItemDefinitions) {
  const reject: Array<keyof ArmorItemDefinitions> = []
